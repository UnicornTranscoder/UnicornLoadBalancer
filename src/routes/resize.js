...
RoutesResize.proxy = (req, res) => {
  ...
  const proxy = httpProxy.createProxyServer({
    target: config.custom.image.proxy,
    changeOrigin: true,
  });
  proxy.on("error", (e) => {
    return res
      ...
      .send({
        error: {
          code: "RESIZE_ERROR",
          message: "Invalid parameters, resize request fails",
        },
      });
  });
  proxy.web(req, res);
...
RoutesResize.resize = (req, res) => {
  ...
  // Process image
  resize(params, req.headers)
    .then((stream) => {
      // Mime type
      if (params.format === "webp") res.type(`image/webp`);
      else if (params.format === "png") res.type(`image/png`);
