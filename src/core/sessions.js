...
SessionsManager.chooseServer = async (session, ip = false) => {
  if (urls[session]) return urls[session];
  let url = "";
  try {
    url = await ServersManager.chooseServer(session, ip);
  } catch (err) {}
  D("SERVER " + session + " [" + url + "]");
  if (url.length) urls[session] = url;
  return url;
};

SessionsManager.cacheSessionFromRequest = (req) => {
  if (
    typeof req.query["X-Plex-Session-Identifier"] !== "undefined" &&
    typeof req.query.session !== "undefined"
  ) {
    cache[
      req.query["X-Plex-Session-Identifier"]
    ] = req.query.session.toString();
  }
};

SessionsManager.getCacheSession = (xplexsessionidentifier) => {
  if (cache[xplexsessionidentifier]) return cache[xplexsessionidentifier];
  return false;
};

SessionsManager.getSessionFromRequest = (req) => {
  if (typeof req.params.sessionId !== "undefined") return req.params.sessionId;
  if (typeof req.query.session !== "undefined") return req.query.session;
  if (
    typeof req.query["X-Plex-Session-Identifier"] !== "undefined" &&
    typeof cache[req.query["X-Plex-Session-Identifier"]] !== "undefined"
  )
...
SessionsManager.parseFFmpegParameters = async (args = [], env = {}) => {
  ...
  const sessions = args
    ...
    .map((e) => e.match(regex)[1]);
  const sessionFull = typeof sessions[0] !== "undefined" ? sessions[0] : false;
  const sessionId =
    typeof sessions[0] !== "undefined" ? sessions[0].split("/")[0] : false;

  // Check Session Id
  if (!sessionId || !sessionFull) return false;
  ...
  const parsedArgs = args.map((e) => {
    ...
    // Other
    return e
      .replace(plexUrl(), publicUrl())
      .replace(config.plex.path.sessions, publicUrl() + "api/sessions/")
      .replace(config.plex.path.usr, "{INTERNAL_RESOURCES}");
  });

  // Add seglist to arguments if needed and resolve links if needed
  const segList =
    "{INTERNAL_TRANSCODER}video/:/transcode/session/" +
    sessionFull +
    "/seglist";
  let finalArgs = [];
  ...
  for (let i = 0; i < parsedArgs.length; i++) {
    ...
    if (
      i > 0 &&
      parsedArgs[i - 1] === "-i" &&
      !config.custom.download.forward
    ) {
      let file = parsedArgs[i];
      try {
        const data = await Database.getPartFromPath(parsedArgs[i]);
        if (typeof data.id !== "undefined")
          file = `${publicUrl()}library/parts/${
            data.id
          }/0/file.stream?download=1`;
      } catch (e) {
        file = parsedArgs[i];
      }
      finalArgs.push(file);
...
SessionsManager.storeFFmpegParameters = async (args, env) => {
  ...
  SessionStore.set(parsed.session, parsed)
    .then(() => {})
    .catch(() => {});
  return parsed;
};
